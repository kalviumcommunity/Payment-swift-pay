...
import Offline from './Offineline'; // Import the OfflinePage component
...
function LandingPage() {
  ...
  // Render the offline page if there is no internet connection
  if (!navigator.onLine) {
    return <OfflinePage/>;
  }

  // Render the loading spinner while content is loading
  if (loading) {
    return <Loading />;
  }

  // Render the main content if loading is complete
  return (
    <>
      {/* Left section */}
      <div className="sm:mb-10 md:flex flex-col md:flex-row justify-between items-start h-screen mt-10">
        <div className={`md:w-1/2 px-8 md:pr-0 mt-10 md:mt-0 ${showContent ? 'slide-in-left' : 'hidden'} ${!showContent ? 'laptop-view' : ''}`}>
          <div className="flex flex-col justify-center items-center h-full">
            <h1 class="M" className="text-3xl md:text-5xl font-bold mb-4 md:mb-6 mt-5 md:mt-10 leading-tight text-center">
        ...
        <div className={`md:w-1/2 hidden md:block flex justify-center items-center mt-10 md:mt-0 overflow-hidden ${showContent ? '' : 'opacity-0'}`}>
          <Tilt tiltMaxAngleX={5} tiltMaxAngleY={5} scale={1.05} gyroscope={true}>
            <img src={head} class='head-image' alt="Head" className="mx-auto max-w-2/3 md:max-w-full max-h-64 md:max-h-full mt-10" />
          </Tilt>
        </div>
      </div>
